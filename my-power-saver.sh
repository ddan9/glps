...
#                                                                                                                 #
# 4) Add comments for your code (every block of code)                                                             #
#                                                                                                                 #
###################################################################################################################
...
#                                                                                              #
# You can use editor search to find needed things                                              #
#                                                                                              #
################################################################################################
...
use_custom_battery_life_extender="1" # ATTENTION! You must check this item yourself in your system; battery life (cycle) extender funcion in BIOS if hardware support; depends of polling time; statements: 0/1 #
	...
	battery_life_extender_enable_percentage="50" # enable battery life extender after that battery percentage; statements: number #
...
use_changing_online_cores="1" # online cores statement change function, can enable or disable cores; statements: 0/1 #

	battery_number_of_online_cores="1" # enabled number of cores on battery; statements: 1/number/auto #
...
use_changing_governor="1" # cpu cores governor change function; statements: 0/1 #
	...
	battery_governor="powersave" # ATTENTION! You must check this item yourself in your system #
...
use_systemd="1" # parameter that provides the ability to control systemd processes; depends of systemd package; statements: 0/1 #
	...
	disable_teamviewerd="1" # disable teamviewed service using systemd if you have it; statements: 0/1 #
...
# function of pre-anywayshot disabling teamviewerd service with systemd (start) #

function pre_anywayshot.systemd.disable_teamviewerd()

{

	if [ $use_systemd = "1" ] && [ $disable_teamviewerd = "1" ]

		then

			{

				if [[ $(systemctl status teamviewerd.service | grep -c "disabled") != "1" ]]
				...
				if [[ $(systemctl status teamviewerd.service | grep -c "inactive") != "1" ]]
...
# function of pre-anywayshot disabling teamviewerd service with systemd (end) #
	...
	do

		{
...

			{
				...
				pre_anywayshot.systemd.disable_teamviewerd; # pre-anywayshot disable teamviewerd service with systemd #
...

{
	...
	# main functions calls (start) #

	main.oneshot;

	main.daemon;

	# main functions calls (end) #
